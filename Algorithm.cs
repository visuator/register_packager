...
public class Algorithm
{
    ...
    {
        return JoinRecursive(max, Chunk(max, regs).ToArray(), 0).Where(x => x.Length != 0).ToArray();
    }
    /*
     ...
     */
    private static int[][] JoinRecursive(int max, int[][] chs, int i)
    {
        ...
        while (i < chs.Length)
        {
            ...
            if (i + 1 < chs.Length)
            {
                ...
                foreach (var (tl, jr) in Combine(cur, fl))
                {
                    ...
                    {
                        if (ExcessLimit(max, jr, out var taken, out var rest))
                        {
                            return JoinRecursive(max, [..chs[..i], tl, taken, rest, ..chs[(i + 2)..]], i + 2);
                        }
                        chs = [..chs[..i], tl, [..taken], ..chs[(i + 2)..]];
                        i++;
                        break;
                    }
                }
    ...
    public static bool ExcessLimit(int max, int[] ch, out int[] taken, out int[] rest)
    {
        ...
        static bool ExcessLimit(int max, int[] chunk) => chunk[^1] - chunk[0] + 1 > max;
    }
